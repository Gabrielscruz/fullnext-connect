...
export default function Update({ params }: any) {
  ...
  const getPowerBiCredential = useCallback(async (id: number) => {
    try {
      const { status, data } = await api.get(`/powerbi/credential/${id}`)
      if (status === 200) {
        const { name, clientId, clientSecret, tenantId } = data.powerBiCredential
        setValue('name', name)
  ...
  useEffect(() => {
    if (params?.id) getPowerBiCredential(params?.id)
  }, [params?.id])
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const response = await api.put(`/powerbi/credential/${params.id}`, data);

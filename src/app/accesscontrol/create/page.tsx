...
export default function Create() {
  ...
  const selectAll = (checked: boolean) => {
    const checkedAll = links.map((link) => ({ ...link, checked }));
    setLinks(checkedAll);
  };
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      const formData = {
        ...data,
        links: links.filter((link) => link.checked).map((link) => ({
          menuLinkId: link.id,
        })),
      };
      const response = await api.post('accesscontrol', formData);
      ...
      <form onSubmit={onSubmit}>
        <Row isBorderActive>
          ...
          <Col classNameColSpan={['col-span-12', 'md:col-span-6', 'lg:col-span-6']}>
            <Button
              type="button"
              className="bg-blue-500 hover:bg-blue-700 w-20 text-white"
          ...
          <Col classNameColSpan={['col-span-12', 'md:col-span-6', 'lg:col-span-6']}>
            <Button
              type="button"
              className="bg-red-500 hover:bg-red-700 w-20 text-white"
